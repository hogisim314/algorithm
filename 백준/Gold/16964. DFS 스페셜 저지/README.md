...
### 해설 
전형적인 dfs문제처럼 보이지만, dfs를 문제에서 쓰진 않는다. 그냥 dfs를 개념적으로 완벽히 알아야 하고, 살짝 트리 문제 같다. 노드의 자식 노드에 해당 번호가 있는지 찾아야 한다. 빠르게 찾기 위해 이분탐색을 했다. 중요한 것은 이분탐색을 하기 위해서는 정렬을 해야한다. 이걸 까먹어서 5번 틀림

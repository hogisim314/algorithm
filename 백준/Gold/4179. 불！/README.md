...
### 해설
이 문제는 아이디어가 중요한 문제 같다. 지훈이를 bfs로 움직이면서 불을 bfs로 움직이면 시간상태(?)가 안맞아서 도저히 풀 수가 없다. 그래서 각각 따로 bfs로 움직여서 각 좌표별로 도착한 시간을 기록한 후 시간을 비교하는 식으로 해야한다. 그리고 잦은 예외처리를 해주면 통과

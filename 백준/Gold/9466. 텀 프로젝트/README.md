...
### 해설
 <p>꽤나 어렵다. 먼저 코드를 조금 더럽게 짰지만, 한번 방문한 건 visited1 배열에 넣었고, 다시 visited1배열에 도착했을 때는 싸이클 시작으로 인지해서 visited2배열에 넣었다. 그러면서 stack에 넣어서 집합의 부모를 group배열에 넣어주었다. </p>

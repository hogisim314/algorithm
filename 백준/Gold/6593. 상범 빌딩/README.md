...
### 해설
이 문제는 간단한 bfs문제인데, 이 문제에서 주목할것은 메모리 초과이다. 처음에 33*33*33 짜리 배열을 3개나 만들었다. building, visited, minute 배열을 만들었는데 128MB초과가 났다. 
int가 4B고 내가 33*33*33*3개를 만들었으니까 총 431244B이다. 1MB는 10^6B니까 100000이고, 128MB면 128000000인데 왜 메모리 초과가 나는지 모르겠다...
중요한 것은 visited배열 푸쉬를 q에 넣을 때 해야한다는 점이다. 나중에 q에 들어갔을 때 visited체크를 해버리면 그 전 재귀함수콜에서 for문을 돌면서 같은 지점을 여러군데에서 q에 넣을 수 있기 때문이다. 
